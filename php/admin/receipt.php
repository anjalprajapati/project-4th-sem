...
requireRole('admin');
$id = (int)($_GET['id'] ?? 0);
$d = dbFetchOne("SELECT d.*, u.name AS donor_name, u.email AS donor_email, c.title AS campaign_title FROM donations d JOIN users u ON d.user_id=u.id JOIN campaigns c ON d.campaign_id=c.id WHERE d.id=?", [$id], 'i');
if (!$d) { redirect(SITE_URL . '/php/admin/donations.php', 'Donation not found.', 'error'); }
$rootPath = '../../';
?>
<!DOCTYPE html>
<html lang="en">
